...
from shop.services.source_bulk_job import (
    ...
    MAX_WARNINGS,
    _existing_urls,
    _import_unpriced_catalog_product,
    read_job,
    write_job,
)
from shop.services.source_catalog import CatalogSkip, upsert_source_product_with_changes
...
def run_full_sync(job_id):
    """Run full catalog sync without allowing catalog discovery to stall at 0/0."""
            ...
            try:
                if site.bulk_import_enabled:
                    ...
                    def progress(info):
                        ...
                        state["discover_budget"] = int(info.get("budget") or 0)
                    ...
                    discovered, meta = discover_product_urls_bounded(site, progress=progress)
                    ...
                    state["discover_budget"] = int(meta.get("budget") or 0)
                    if meta.get("timed_out") and len(state["warnings"]) < MAX_WARNINGS:
                        state["warnings"].append(
                            ...
                            f"{len(discovered)} محصول پیدا شده + محصولات قبلی همگام شدند."
                        )
