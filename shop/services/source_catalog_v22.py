...
from shop.services.category_v22 import canonical_path, enhanced_category_names, sync_category_path
from shop.services.source_identity_v22 import (
    aggregate_product,
...
# Use the exact source breadcrumb parser for every v22 scrape.
...
def scrape_product_with_retry(url, attempts=None):
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            data["categories"] = canonical_path(
...
def _upsert_with_data(site, url, data, allow_unpriced=False):
    ...
    sale_price = _markup_price(
        incoming_price,
        site.default_markup_type,
        site.default_markup_value,
    ) if incoming_price else 0
    ...
    if offer:
        ...
        if (
            ...
            and not ProductSourceOffer.objects.exclude(pk=offer.pk).filter(source_url=canonical_url).exists()
        ):
            offer.source_url = canonical_url
        offer.save()
    else:
    ...
    if not incoming_price:
        Product.objects.filter(pk=product.pk).update(
            sync_error="قیمت این منبع فعلاً قابل استخراج نیست؛ موجودی سایر منابع حفظ شد."
        )
...
def upsert_source_product_with_changes(site, url):
    ...
    existing = (
        offer.product if offer
        else find_canonical_product(model_key)
        or _legacy_product(site, canonical_url, data)
        or _legacy_product(site, url, data)
    )
    ...
    if created:
        ...
        for field in (
            "name", "source_price", "price", "stock", "image_url", "gallery",
            "specs", "category_id", "active",
        ):
...
# Existing API/bot modules still import the legacy module. Upgrade only those
# public call points; v21's own module remains untouched for regression safety.
